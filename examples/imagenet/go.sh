...
CMD="$PROFILECMD python ${MAIN} --arch ${ARCH} --batchsize ${BATCHSIZE} --gpu ${GPU} ${OOCOPT} ${AUTOOPT} ${AUTOWSOPT} ${MEMTRACEOPT} ${ITEROPT} --loaderjob 4 --val_batchsize ${VAL_BATCHSIZE} ${TRACEOPT} --memprof ${MEMPROF} --memcallstack ${MEMCALL} --memprint ${MEMPRINT} --timer ${TIMER} --printvar ${PRINTVAR} --ooc ${OOC} --usepool ${USEPOOL} -o ${LOGBASE}.result ${DATAARGS}"
