...
MAIN=${MAIN:-"train_imagenet_data_parallel_sym.py"}
...
LOGBASE="${LOGDIR}/${TIME}-${ARCH}-`printf %04d ${BATCHSIZE}`${OOCOPT}${LWROPT}"
[ "${PROFILEOPT}" != "0" ] && PROFILECMD="${PROFILEPREFIX} -o $LOGBASE.nvvp"

CMD="$PROFILECMD python ${MAIN} --arch ${ARCH} --batchsize ${BATCHSIZE} --gpus "${GPUS}" ${OOCOPT} ${LWROPT} ${MEMTRACEOPT} --epoch ${EPOCH} --loaderjob 4 --val_batchsize ${VAL_BATCHSIZE} --root ${DATA_ROOT} -o ${LOGBASE}.result ${DATAARGS}"
