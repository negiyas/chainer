...
CMD="$PROFILECMD python ${MAIN} --batchsize ${BATCHSIZE} --iteration ${ITER} --epoch ${EPOCH} --gpu ${GPU} ${MEMTRACEOPT} --noplot --memprof ${MEMPROF} --memcallstack ${MEMCALL} --timer ${TIMER} --ooc ${OOC} --usepool ${USEPOOL} -o ${LOGBASE}.result"
